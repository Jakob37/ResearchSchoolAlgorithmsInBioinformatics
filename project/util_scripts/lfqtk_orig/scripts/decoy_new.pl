...
#############################################################################
# Modified version of decoy.pl from Matrix Science. Special naming of	    #	
# IPI random or reverse. 2006-10-02 Fredrik Levander Lund University	    #
#									    #	
...
# print usage information if no arguments supplied  
  unless ($ARGV[0]) {
    ...
    print "   IPI entries will instead be renamed to IPRND ord IPREV \n";
...
# loop through input file and create the output file
  ...
  # remove any non-residue characters from input sequence
    ...
    if ($random) {
	  ...
	  if ($accession =~ /IPI:/i){
		my ($acc, $des) = split (/[\s|]/,$accession,2);
		$acc =~ s/:IPI/:IPRND/i;
		print OUTFILE ">" . $acc . " Random sequence, was " . $des . " " . $description . $eol;  
	  } else {
        	print OUTFILE ">r" . $accession . " Random sequence, was " . $description . $eol;
	  }
      }
    ...
    # chop into little pieces for output
      ...
      if ($keep_accessions) {
        ...
        if ($accession =~ /IPI:/i){
                my ($acc, $des) = split (/[\s|]/,$accession,2);
                $acc =~ s/:IPI/:IPREV/i;
                print OUTFILE ">" . $acc . " Reverse sequence, was " . $des . " " . $description . $eol;
        } else {
		print OUTFILE ">r" . $accession . " Reverse sequence, was " . $description . $eol;
        }
      }
